  ...
  class Config
    class << self
      ...
      def hunter_command
        ...
        if !File.file?(File.join(command.split.join("/")))
          raise "Could not find '#{command.split[1]}' in #{command.split[0]}"
        elsif !File.executable?(File.join(command.split.join("/")))
          raise "#{command.split[1]} is not executable"
        end
        command.split(' ')
      end
